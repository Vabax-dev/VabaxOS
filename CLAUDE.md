...
- In corso, blocco 2 (ramo `feat/block2-setup-installer`): menu Accessibilità e Installa nel benvenuto (velocità della voce, testo grande, contrasto, zoom, tasti permanenti); programma `vabaxos-setup` (GTK 4, 5 pagine, letto da Orca); installer con voce (tasto I) e altre modalità (O: alto contrasto, grafica, testo); `vabaxos-a11y-check`; CI `iso.yml` che costruisce e avvia la ISO; logo di Vabax vettoriale (`artwork/`, `scripts/lib/trace-logo.py`, filigrana tolta) nel menu di avvio con tema ad alto contrasto; pacchetto `vabaxos-branding` (icona `vabaxos-logo`, sfondo, logo in Informazioni via os-release deviato). Snapshot spostato al 20260924 (kernel 7.2.6: il debian-installer di git lo richiede). Correzioni finali: live-build da Git (commit 531cdb98), installer con voce avviato dall'initrd grafico (solo lì ci sono espeakup e i driver audio), os-release anche in `/etc`. `test-boot.sh` passa in tutte e 7 le modalità (voice, it, novoice, recovery, bios, secure-boot, install). **Ascoltato e approvato da Vabax (2026-09-24, sera):** benvenuto, Accessibilità, desktop, configurazione iniziale con Tab, installer che parla. Da fare: PR e merge con CI verde.
- Prossimo: blocco 3 (desktop: menu Start con il logo, ADR per ArcMenu o programma Vabax; suoni di sistema; decisione sulla voce Piper), prova su PC fisico.
