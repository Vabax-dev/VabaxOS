...
- Architettura dell'audio (dopo molte prove): un solo server audio, il PipeWire dell'utente live, attivo dall'avvio (linger, `image/config/includes.chroot_after_packages`). Benvenuto, espeakup e Orca parlano tutti attraverso di lui; il benvenuto ripiega su `aplay -D sysdefault` solo se non c'è un server. espeak-ng (pcaudiolib) suona solo via PulseAudio: da root serve `PULSE_SERVER`.
